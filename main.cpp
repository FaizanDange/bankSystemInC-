...
int account_generator(MYSQL* conn, string email)
{
  ...
  MYSQL_RES* res;
  int qstate;
  ss << "SELECT account_no,email FROM accounts";
  string query = ss.str();
...
int account_validator(MYSQL* conn, int acc)
{
    ...
    ss<<"SELECT account_no FROM accounts";
    string query = ss.str();
    const char* q = query.c_str();
    qstate = mysql_query(conn, q);
    if(qstate == 0)
...
void createaccount(MYSQL* conn)
{
    ...
    ss << "INSERT INTO accounts (name, dob, phoneno, address, district, state, gender, balance, email, password) VALUES ('" << namedb << "', '" << dob << "', '" << phonenum <<"', '" << address <<"', '" << district <<"', '"<< state <<"', '" << gender <<"', "<< balance <<", '"<< email <<"', '"<< pass <<"')";
...
int loginaccount(MYSQL* conn, string emaildb, string passdb)
{
  ...
  int qstate = 0;
  int flag = 0;
  ss << "SELECT account_no,email,password FROM accounts";
  string query = ss.str();
  const char* q = query.c_str();
  qstate = mysql_query(conn, q);
  if(qstate == 0){
...
void display_details(MYSQL* conn,int acc)
{
  ...
  MYSQL_RES* res;
  int qstate;
  ss << "SELECT account_no, name, dob, phoneno, address, district, state, gender, balance, email FROM accounts";
  string query = ss.str();
  const char* q = query.c_str();
  qstate = mysql_query(conn, q);
  if(qstate == 0){
...
int bal_validate(MYSQL* conn, int acc, int transfer)
{
    ...
    ss<<"select balance from accounts where account_no = "<< acc <<"";
    string query = ss.str();
    const char* q = query.c_str();
    qstate = mysql_query(conn, q);
...
void transfer_money(MYSQL* conn, int acc)
{
  ...
  ss<<"SELECT account_no,balance FROM accounts";
  string query = ss.str();
  const char* q = query.c_str();
  qstate = mysql_query(conn, q);
    ...
    while((row = mysql_fetch_row(res)))
    {
        ...
        if(acc == ac)//For Sender Balance Update
        {
            if(bala > transfer_amt)
            {
              ...
              ss1 << "UPDATE accounts SET balance = "<< sender_amt <<" WHERE account_no = "<< acc <<"";
        ...
        if(account_num == ac1)//For Recipient Balance Update
        {
              ...
              ss2 << "UPDATE accounts SET balance = "<< recipient_amt <<" WHERE account_no = "<< account_num <<"";
...
void deposit(MYSQL* conn, int acc_no)
{
    ...
    if(spin == 1)
    {
        ss1<<"select balance FROM accounts WHERE account_no = "<< acc_no <<"";
        ...
        if(qstate1 == 0)
        {
            ...
            ss2<<"UPDATE accounts SET balance = "<< bal <<" WHERE account_no = "<< acc_no <<"";
            string query2 = ss2.str();
            const char *q2 = query2.c_str();
            qstate2 = mysql_query(conn,q2);
            if(qstate2 == 0)
...
void show_pin(MYSQL* conn,int acc_no)
{
    ...
    ss<<"SELECT pin FROM mpin, accounts WHERE (mpin.account_no = "<< acc_no <<" AND accounts.account_no = "<< acc_no <<") AND accounts.email = '" << email << "'";
    string query = ss.str();
    const char *q = query.c_str();
    qstate = mysql_query(conn,q);
    int flag = 0;
...
int main()
{
    ...
    conn = mysql_real_connect(conn, "192.168.1.87", "admin", "admin", "bank2", 3306, NULL, 0);
    ...
    system("Color B4");//B is background color yellow and 4 is text color red

    if(conn){
        char ch;
        cout<<"Database Connected Successfully. . ."<<endl<<endl<<endl;
        do
        {
            cout<<"\t\t\t=================================================================="<<endl;
